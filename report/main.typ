...
#place(
  ...
  clearance: 2em,
  {
    ...
    table(
      ...
      stroke: none,
      align: horizon,
      image("ens_lyon.png", width:50%),
      image("irif.png", width:50%)
    )

    v(1fr)
    text(size:16pt)[
      Zoé Garreau \
      #text(size: 13pt)[ENS de Lyon]
    ]
    v(0.5fr)
    text(size:16pt)[
      #text(size: 14pt)[Supervised by:] \
      Giuseppe Castagna \
      #text(size: 13pt)[CNRS, IRIF]
    ]
    v(1fr)
    text(size: 14pt)[
      Internship at IRIF \
...
Finally, a check is added to ensure that the type generated by a `:>` subclass is a subtype:
...
A class `A` generates a top-level statement of the form (excluding some type coercions and mutability declarations for clarity):
...
The `new` class method has a particular translation of the form (with the same omissions):
...
*Definition.* We define the _domain_ and _codomain_ functions for RBS types:
...
in @BruEtAl96: since subclasses are no longer subtypes,
simple subtyping constraints are not as useful for polymorphism.
Gradual typing is also essential to type dynamic languages to allow the programmer
to migrate their untyped code progressively instead of requiring the whole program to be
